...
public class FactoryUnitBuildState : IBuildingState {
    ...
    public FactoryUnitBuildState(BuildingFactory building)
    {
    ...
    IEnumerator InstantiateUnit(int unitID)
    {
        ...
        {
            //spawnLocation = UnitLocationsManager.FindClosestFreePlaneTo(building.gameObject.transform.position);
            spawnLocation = UnitLocationsManager.FindLocation(building.transform.position, 100, false);
            ...
            {
                NotificationSystem.SendNotification("Debug Alert!", "A Factory cannot spawn an unit", Color.yellow, building.transform.position);
        ...
        if (building.levelMaster.HumanPlayer)
            NotificationSystem.SendNotification("Unit created", "A new Collector has been created", Color.green, building.transform.position);
        building.levelMaster.ExpectedCollectorCount--;
    }
